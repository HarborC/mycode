...
class PointOdysseyAdapter(BaseAdapter):
    ...
    def get_sequence_info(self, sequence_name: str) -> dict[str, Any]:
        ...
        return {
            ...
            "has_tracks": r.anno_path is not None or r.fast_anno_paths is not None,
            "has_visibility": r.anno_path is not None or r.fast_anno_paths is not None,
            "has_trajs_3d_world": r.anno_path is not None or r.fast_anno_paths is not None,
    ...
    def load_clip(self, sequence_name: str, frame_indices: list[int]) -> UnifiedClip:
        ...
        if r.encoded_cache_paths is not None:
            ...
            # Fix depth scale: PointOdyssey depths are ~100x too large
            depths = [d * 0.01 if d is not None else None for d in depths]
        ...
        # Fix depth scale: PointOdyssey depths are ~100x too large (stored in cm or dm)
    ...
    def _decode_normal_from_cache(self, cache: dict, frame_pos: int) -> Optional[np.ndarray]:
        ...
        if suffix == ".npy":
            return np.load(io.BytesIO(encoded.tobytes())).astype(np.float32)
        normal = cv2.imdecode(encoded, cv2.IMREAD_UNCHANGED)
        if normal is None:
            return None
        if normal.ndim == 2:
            return normal.astype(np.float32)
        return cv2.cvtColor(normal, cv2.COLOR_BGR2RGB).astype(np.float32)
