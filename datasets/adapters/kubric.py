...
class KubricAdapter(BaseAdapter):
    ...
    def __init__(self, root: str, split: str = "train", cache_dir: Optional[str] = None):
        """
        Args:
    ...
    def get_num_frames(self, sequence_name: str) -> int:
        """Skip per-sequence .npy loading; assume sequences have sufficient frames.

        Kubric sequences are guaranteed to have enough frames for the configured
    ...
    def load_clip(self, sequence_name: str, frame_indices: list[int]) -> UnifiedClip:
        ...
        if h5_path.exists():
            ...
            if depth_files:
                with ThreadPoolExecutor(max_workers=min(len(frame_indices), 8)) as ex:
                    depths = list(ex.map(
                        lambda i: np.load(depth_files[i]).astype(np.float32).squeeze(),
                        frame_indices))
            else:
                depths = []
        ...
        with ThreadPoolExecutor(max_workers=min(len(frame_indices), 8)) as ex:
            images = list(ex.map(lambda i: self._read_image(frame_files[i]), frame_indices))
