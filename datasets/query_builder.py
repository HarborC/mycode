...
class D4RTQueryBuilder:
    ...
    def _sample_with_tracks(
        ...
        for fi in range(T):
            ...
            # 仅仅保留处于相机前方的点，真正的有效性由 valids 和 visibs 保证
            ...
            depth_valid = (depth_z > 1e-3) & (depth_z < 10000) & np.isfinite(depth_z)

            valid_src = np.where(
                (valids[fi] > 0.5) & (visibs[fi] > 0.5) & in_bounds & world_finite & depth_valid
            )[0]
        ...
        cam_frames   = np.where(use_same_cam, tgt_frames, np_rng.integers(0, T, size=N))
        ...
        # 仅仅确保点在相机前方且数值合法（与line 375保持一致）
        src_depth_valid = (src_cam[:, 2] > 1e-3) & (src_cam[:, 2] < 10000) & np.isfinite(src_cam[:, 2])
        tgt_depth_valid = (tgt_cam[:, 2] > 1e-3) & (tgt_cam[:, 2] < 10000) & np.isfinite(tgt_cam[:, 2])
        ...
        if result.normals is not None and result.normal_valids is not None:
            ...
            active      = vis_flag & valid_nf[tgt_frames]
            if active.any():
                ...
                targets["mask_normal"][qi] = torch.isfinite(targets["normal"][qi]).all(-1)
    ...
    def _sample_no_tracks(
        ...
        for fi in range(T):
            ...
            valid_mask = np.isfinite(depth) & (depth > 1e-3) & (depth < 10000)
        ...
        for qi in range(self.num_queries):
            ...
            if has_depth:
                ...
                if depth_val > 1e-3 and depth_val < 10000 and np.isfinite(depth_val):
                    K_src = K[src_fi]
                    ...
                    if np.isfinite(p_cam).all() and p_cam[2] > 1e-3 and p_cam[2] < 10000:
