...
def compute_confidence_loss(
    ...
    Full loss per point: exp(-s) * L_3D + s.
    This function computes only the penalty term: s = log(σ²).
...
class D4RTLoss(nn.Module):
    ...
    def _reduce_point_loss(
        ...
        """Apply Kendall confidence weighting to 3D loss with gradual ramp-up.

        Full Kendall loss per point: exp(-s) * L_3D + λ_conf * s
        This method handles the exp(-s) * L_3D part; the λ_conf * s penalty
        is computed separately in compute_confidence_loss().
        """
        if self.use_confidence_weighting and self.lambda_conf > 0:
            # s = log(σ²); weight = exp(-s) = 1/σ²: down-weights uncertain predictions
