    ...
    def load_clip(self, sequence_name: str, frame_indices: list[int]) -> UnifiedClip:
        ...
        if cache is not None:
            try:
                ...
                # trajs_3d_world in the cache stores ref-frame world coords broadcast
                # to all frames (all frames identical). Recompute per-frame by
                # backprojecting trajs_2d through the depth map, following Waymo adapter.
                T = len(frame_indices)
                N_pts = trajs_2d_out.shape[1]
                trajs_3d_out = np.zeros((T, N_pts, 3), dtype=np.float32)
                for ti in range(T):
                    ...
                    if depth_t is not None:
                        ...
                        z = depth_t[ys, xs].astype(np.float64)
                        # valid: depth in (0, 80m] — removes sky/invalid pixels
                        valid = (z > 0.0) & (z < 80.0)
                        valids_out[ti] = valid

                        x_c = (xs - cx) / fx * z
                        y_c = (ys - cy) / fy * z
                        pts_cam = np.stack([x_c, y_c, z, np.ones(N_pts)], axis=1)  # [N,4]
                        pts_world = (E_inv @ pts_cam.T).T[:, :3]                   # [N,3]
                        trajs_3d_out[ti] = pts_world.astype(np.float32)
                    else:
                        valids_out[ti] = False
