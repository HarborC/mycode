...
class Co3Dv2Adapter(BaseAdapter):
    ...
    def __init__(
        ...
        # sequence_uid → PLY point cloud [N,3] in world space (cached in memory)
        self._ply_cache: dict[str, np.ndarray] = {}
    ...
    def load_clip(self, sequence_name: str, frame_indices: list[int]) -> UnifiedClip:
        ...
        # Load PLY point cloud (cached per sequence) for depth rendering.
        # Load PLY pointcloud (cached per sequence, used for track generation if needed)
        ply_path = r.sequence_dir / "pointcloud.ply"
        if sequence_name not in self._ply_cache:
            try:
                from datasets.computer.co3d_pointcloud_to_tracks import read_ply_pointcloud
                self._ply_cache[sequence_name] = read_ply_pointcloud(ply_path)
            except Exception:
                self._ply_cache[sequence_name] = np.zeros((0, 3), dtype=np.float32)
        ply_pts = self._ply_cache[sequence_name]
