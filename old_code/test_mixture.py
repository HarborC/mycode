#!/usr/bin/env python3
"""Test and visualize D4RT mixture dataset training."""
...
def load_model(checkpoint_path: Path, device: torch.device):
    ...
    # Mixture training uses fixed architecture
    model = create_d4rt(
        encoder="base",
        decoder_depth=6,
        img_size=256,
    ...
    # Load weights (mixture training saves as 'model')
...
def compute_metrics(pred_2d, gt_2d, pred_3d, gt_3d, pred_vis, gt_vis, mask_2d, mask_3d, mask_vis):
    ...
    # 2D tracking error
    if mask_2d.sum() > 0:
        # Normalized coordinates [0,1] - same as training loss (L1 per dimension)
        error_2d_norm = np.abs(pred_2d - gt_2d)  # [N, 2]
        metrics['2d_norm_mean'] = error_2d_norm[mask_2d].mean()
        metrics['2d_norm_median'] = np.median(error_2d_norm[mask_2d])

        # Pixel space (Euclidean distance, assuming 256x256)
        error_2d_px = np.linalg.norm((pred_2d - gt_2d) * 256, axis=-1)
        metrics['2d_px_mean'] = error_2d_px[mask_2d].mean()
        metrics['2d_px_median'] = np.median(error_2d_px[mask_2d])

    # 3D error (meters)
    if mask_3d.sum() > 0:
        error_3d = np.linalg.norm(pred_3d - gt_3d, axis=-1)
        metrics['3d_mean'] = error_3d[mask_3d].mean()
        metrics['3d_median'] = np.median(error_3d[mask_3d])

    # Visibility accuracy
    if mask_vis.sum() > 0:
        vis_pred_binary = (pred_vis > 0.5).astype(float)
        vis_gt_binary = (gt_vis > 0.5).astype(float)
        metrics['vis_acc'] = (vis_pred_binary[mask_vis] == vis_gt_binary[mask_vis]).mean()

    return metrics

def test_sample(model, batch, device, output_dir, sample_idx):
    ...
    pred_2d = outputs['pos_2d'][0].float().cpu().numpy()  # [Q, 2]
    pred_3d = outputs['pos_3d'][0].float().cpu().numpy()  # [Q, 3]
    pred_vis = outputs['visibility'][0].float().cpu().numpy()  # [Q]
    ...
    # Convert video to RGB numpy
    video = batch['video'][0].float().cpu().numpy()  # [S, 3, H, W]
    if video.dtype == np.float32 or video.dtype == np.float64:
        video = (video * 255).astype(np.uint8)
    video_rgb = video.transpose(0, 2, 3, 1)  # [S, H, W, 3]
    ...
    # Print debug info
    print(f"\n[Sample {sample_idx}] {dataset_name}/{seq_name}")
    print(f"  Queries: {len(pred_2d)}, Visible GT: {mask_vis.sum()}")
    print(f"  2D norm: {metrics.get('2d_norm_mean', 0):.4f} (training loss scale)")
    print(f"  2D px: {metrics.get('2d_px_mean', 0):.2f}px, 3D: {metrics.get('3d_mean', 0):.4f}m, Vis: {metrics.get('vis_acc', 0):.3f}")
...
def main():
    ...
    print(f"Dataset: {dataset.get_dataset_names()}")
    ...
    for batch_idx, batch in enumerate(tqdm(loader)):
        ...
        # Convert video to float if needed
        if batch['video'].dtype == torch.uint8:
            batch['video'] = batch['video'].float() / 255.0

        # Test
        metrics = test_sample(model, batch, device, output_dir, batch_idx)
        all_metrics.append(metrics)

        # Print progress
        print(f"Sample {batch_idx}: {metrics['dataset']}/{metrics['sequence']} - "
              f"2D_norm: {metrics.get('2d_norm_mean', 0):.4f}, "
              f"2D_px: {metrics.get('2d_px_mean', 0):.2f}px, "
              f"3D: {metrics.get('3d_mean', 0):.4f}m, "
              f"Vis: {metrics.get('vis_acc', 0):.3f}")
    ...
    if all_metrics:
        avg_metrics = {}
        for key in ['2d_norm_mean', '2d_norm_median', '2d_px_mean', '2d_px_median', '3d_mean', '3d_median', 'vis_acc']:
            values = [m[key] for m in all_metrics if key in m]
            if values:
                avg_metrics[key] = np.mean(values)

        print("\n=== Aggregate Metrics ===")
        print(f"2D (normalized [0,1], L1): {avg_metrics.get('2d_norm_mean', 0):.4f} (compare with training loss)")
        print(f"2D (pixels, Euclidean): {avg_metrics.get('2d_px_mean', 0):.2f}px")
        print(f"3D (meters): {avg_metrics.get('3d_mean', 0):.4f}m")
        print(f"Visibility accuracy: {avg_metrics.get('vis_acc', 0):.3f}")
